...
## Randomly sample parameter vector from priors
prior.samp <- function(chains) {
  l <- lapply(1:chains, function(x) {
    lst <- list(
      ...
      'TL' = rnorm(1, 284, 2),
      'HL' = rgamma(1, 6, scale = 2),
      'TH' = rnorm(1, 304, 2),
      'HH' = rgamma(1, 10, scale = 3),
...
for (s in stages) {
  ## Set up input data
  parms <- parms1
  dd <- subset(all.days.df, province == p & stage == s)
  ...
  ## Get gradient and objective functions from TMB code
  ...
  ## Ensure objective function is finite at random starting values
  parm.lst <- prior.samp(100)
  parm.lst <- lapply(parm.lst, function(x) {
    sub <- subset(dd, temp1 == 25)
    x$upsilon <- rep(0, length(unique(dd2$block)))
    return(x)
  })
  w <- which(sapply(parm.lst, function(x) {!is.finite(min(stan.test(dd2, x)$diff))}))
  parm.lst <- parm.lst[-w][1:4]
  
  ## Run No-U-Turn Sampler
  stan1 <- tmbstan(ff, init = parm.lst, silent = TRUE, 
                   chains = chains, iter = 5000, warmup = 1500, seed = 123,
                   control = list(adapt_delta = 0.999, max_treedepth = 15))
  stan.lst <- append(stan.lst, list(stan1))
  ...
  if (length(g) > 1) {g.df <- do.call('rbind', g)}
  else {g.df <- g[[1]]}
  ...
  write.csv(post.df, file = paste0('code/output/simulations/posterior_', p, '_', s, '.csv'),
            row.names = FALSE)
}
