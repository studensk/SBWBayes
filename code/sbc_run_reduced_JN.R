...
# JN: just took the first 1000 individuals to time the loop without taking to long
ptm <- proc.time()
tdiff.lst <- lapply(unique(all.data$ind)[1:1000], function(u) {
...
# JNC's attempt at vectorizing the loop 
ptm <- proc.time()
# First 5000 rows are the first 1000 individuals
all.data.jn  <- all.data[1:5000,] %>%
  select(stage,time1,time2,ind) %>%
  mutate(time1d=time1,time2d=time2) %>%
  # I find that 1 row per individual is easier on my brain
  group_by(ind) %>%
  pivot_wider(names_from = stage,values_from = c(time1,time2,time1d,time2d)) %>%
  ungroup()

all.data.jn %>%
  # This mutate takes care of the first stage
  mutate(time1d_L2 = time1_L2 - 1*(time2_L2 == 0),
         time2d_L2 = time2_L2 - 1*(time2_L2 > 0)) %>%
  # Here I was about to continue with the next larval stage but I noticed that 
  # row$time1d seems to only depend on row$time2 = 0 or >0 in your algorithm as 
  # it seems to have the same values for dat$time2[i-1] >0 or == 0. If this is
  # the case it should probably be outside the test for dat$time2[i-1] and be 
  # vectorized as below
  mutate(time1d_L3 = ifelse(time2_L3 == 0,pmax(time1_L3 -1,0),time1_L3))
...
ptm <- proc.time()
  all.data$time1d <- all.data$time1
  all.data$time2d <- all.data$time2
  tdiff.lst <- lapply(unique(all.data$ind)[1:1000], function(u) {
  dat <- subset(all.data, ind == u)
  dat[1,11] <- ifelse(dat[1,5] == 0,dat[1,11])
  for (i in 1:5) {
    row <- dat[i,]
    if (i == 1) {
      if (row$time2 == 0) {
