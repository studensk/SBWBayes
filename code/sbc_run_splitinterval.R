...
write.csv(all.data, 'data/sim_data_splitinterval.csv')
write.csv(priors.df, 'data/sim_priors_splitinterval.csv')
...
gr.lst1 <- parLapply(cl1, 1:250, function(i) {
  ...
  write.csv(post.df, paste0('code/output/post_iter_splitinterval', i, '.csv'))
  return(stan1)
})
...
write.csv(diag.df, 'code/output/diagnostics_splitinterval.csv')
